...
export default function SettingsPage() {
    ...
    if (loading) return <div className="flex h-[80vh] items-center justify-center"><Loader2 className="animate-spin text-primary h-12 w-12" /></div>;
        ...
        <div className="max-w-6xl mx-auto space-y-8 p-4 md:p-6 pb-24">
            <div className="mb-10">
                ...
                <p className="text-muted-foreground font-bold mt-3 text-sm uppercase tracking-wide">
                    Ajusta los parámetros principales, tasas de cambio y seguridad del sistema.
                </p>
            </div>
           
            <Tabs defaultValue="company" className="w-full">
                <TabsList className="grid w-full grid-cols-4 h-14 bg-secondary/30 p-1 rounded-2xl">
                    <TabsTrigger value="company" className="rounded-xl font-black uppercase text-[10px] md:text-xs data-[state=active]:bg-card data-[state=active]:text-primary">Empresa</TabsTrigger>
                    <TabsTrigger value="rates" className="rounded-xl font-black uppercase text-[10px] md:text-xs data-[state=active]:bg-card data-[state=active]:text-primary">Tasas</TabsTrigger>
                    <TabsTrigger value="fees" className="rounded-xl font-black uppercase text-[10px] md:text-xs data-[state=active]:bg-card data-[state=active]:text-primary">Acceso</TabsTrigger>
                    <TabsTrigger value="security" className="rounded-xl font-black uppercase text-[10px] md:text-xs data-[state=active]:bg-card data-[state=active]:text-primary">Seguridad</TabsTrigger>
                </TabsList>

                <TabsContent value="company" className="mt-6">
                    <Card className="rounded-[2.5rem] border-border bg-card overflow-hidden">
                        <CardHeader className="bg-secondary/20 p-8">
                            <CardTitle className="text-xl font-black uppercase tracking-tight flex items-center gap-3"><Building2 className="text-primary" /> Identidad Visual y Legal</CardTitle>
                        </CardHeader>
                        <CardContent className="p-8 space-y-8">
                            <div className="flex flex-col md:flex-row items-center gap-8">
                                <div className="relative">
                                    ...
                                    <Button size="icon" className="absolute bottom-0 right-0 rounded-full bg-primary hover:bg-primary/90 h-10 w-10 shadow-lg" onClick={() => document.getElementById('logo-input')?.click()}><Upload className="h-4 w-4" /></Button>
                ...
                <TabsContent value="rates" className="mt-6">
                    <Card className="rounded-[2.5rem] border-border bg-card">
                        <CardHeader className="bg-secondary/20 p-8"><CardTitle className="text-xl font-black uppercase">Tasas de Cambio (BCV)</CardTitle></CardHeader>
                        <CardContent className="p-8 space-y-6">
                            <div className="flex flex-col md:flex-row gap-4 items-end bg-muted/50 p-6 rounded-3xl border border-border">
                                ...
                                <div className="flex-1 w-full space-y-1"><Label className="text-[10px] font-black text-muted-foreground">Monto Bs.</Label><Input type="number" className={inputStyle} value={newRate.rate} onChange={e => setNewRate({...newRate, rate: e.target.value})} /></div>
                            ...
                            <div className="rounded-2xl border border-border overflow-hidden">
                                <Table>
                                    <TableHeader className="bg-secondary/30"><TableRow><TableHead className="font-black text-muted-foreground">FECHA</TableHead><TableHead className="font-black text-muted-foreground">TASA</TableHead><TableHead className="font-black text-muted-foreground text-right">ESTADO</TableHead></TableRow></TableHeader>
                                    <TableBody>
                                        {settings.exchangeRates?.map((r) => (
                                            <TableRow key={r.id} className="hover:bg-secondary/20 transition-colors border-border">
                                                ...
                                                <TableCell className="text-right">{r.active ? <Badge variant="success">ACTIVA</Badge> : <Badge variant="outline">HISTORIAL</Badge>}</TableCell>
                                            </TableRow>
                                        ))}
                                    </TableBody>
                                </Table>
                    ...
                    <Card className="rounded-[2.5rem] border-border bg-card">
                        ...
                        <CardContent className="p-8 space-y-8">
                            <div className="space-y-4">
                                <Label className="text-[10px] font-black uppercase text-muted-foreground">Cuota Mensual Sugerida</Label>
                                <div className="flex gap-4 items-center bg-muted/50 p-6 rounded-[2rem] border border-border">
                            ...
                            <div className="p-8 rounded-[2rem] bg-muted/50 border border-border flex items-center justify-between">
                                <div><h3 className="font-black uppercase text-foreground">Acceso para Propietarios</h3><p className="text-xs text-muted-foreground font-bold uppercase tracking-tighter">Habilitar entrada a la App de vecinos.</p></div>
                                <Switch className="data-[state=checked]:bg-primary" checked={settings.loginSettings?.ownerLoginEnabled} onCheckedChange={c => setSettings({...settings, loginSettings: {...settings.loginSettings, ownerLoginEnabled: c}})} />
                            </div>
                        </CardContent>
                        <CardFooter className="p-8 bg-secondary/20 flex justify-end">
                ...
                <TabsContent value="security" className="mt-6">
                    <Card className="rounded-[2.5rem] border-border bg-card overflow-hidden">
                        <CardHeader className="bg-destructive text-destructive-foreground p-8"><CardTitle className="text-xl font-black uppercase flex items-center gap-3"><KeyRound /> Seguridad Crítica</CardTitle></CardHeader>
                        <CardContent className="p-8 space-y-6">
                            <div className="space-y-4">
                                <Label className="text-[10px] font-black uppercase text-muted-foreground ml-2">Nuevo PIN de Autorización</Label>
                                <Input type="password" placeholder="••••••" maxLength={6} className="rounded-2xl text-center text-4xl h-20 tracking-[0.5em] bg-input border-border font-black text-foreground" value={newAuthKey} onChange={e => setNewAuthKey(e.target.value)} />
                                <p className="text-[11px] text-destructive uppercase font-black text-center">Este PIN será necesario para borrar registros o modificar datos sensibles.</p>
                            </div>
                        </CardContent>
                        <CardFooter className="p-8 bg-secondary/20 flex justify-end">
                            <Button className="bg-destructive hover:bg-destructive/90 rounded-full px-10 h-12 font-black text-destructive-foreground shadow-lg" onClick={() => handleSaveSecurity(newAuthKey)} disabled={!newAuthKey || newAuthKey.length < 4}>
