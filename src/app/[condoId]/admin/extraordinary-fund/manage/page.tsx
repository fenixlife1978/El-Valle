...
import { Loader2, Plus, Users, FileText, AlertCircle, DollarSign, XCircle, ChevronDown, ChevronRight, Share2, Download } from 'lucide-react';
...
export default function ManageExtraordinaryFundPage() {
    ...
    const [formData, setFormData] = useState({ description: '', amountUSD: '' });
    ...
    useEffect(() => {
        ...
        const fetchCondominio = async () => {
            const ds = await getDoc(doc(db, 'condominios', condoId));
            if (ds.exists()) setCondominioData(ds.data());
        };
        fetchCondominio();
    }, [condoId]);
    ...
    useEffect(() => {
        if (!condoId) return;
        const campaignsQuery = query(collection(db, 'condominios', condoId, 'extraordinary_campaigns'));
        ...
        const debtsQuery = query(collection(db, 'condominios', condoId, 'owner_extraordinary_debts'));
    ...
    const getCampaignStats = (campaignId: string) => {
        ...
        return { totalAmount, collected: paidAmount + partialPaid, pending: totalAmount - paidAmount - partialPaid };
    };
    ...
    const getSortedOwnersWithProperties = () => {
        ...
        return ownersWithProps.sort((a, b) => {
            ...
            const so: Record<string, number> = { 'Calle 1': 1, 'Calle 2': 2, 'Calle 3': 3, 'Calle 4': 4, 'Calle 5': 5, 'Calle 6': 6, 'Calle 7': 7, 'Calle 8': 8 };
            const sc = (so[aProp.street] || 99) - (so[bProp.street] || 99);
            if (sc !== 0) return sc;
    ...
    const handleCreateCampaign = async () => {
        if (!formData.description || !formData.amountUSD) { toast({ variant: 'destructive', title: 'Error' }); return; }
        setLoading(true);
        try {
            ...
            for (const owner of owners) {
                for (const prop of sortProperties(owner.properties || [])) {
                    ...
                    batch.set(debtRef, {
                        ownerId: owner.id, ownerName: owner.name, property: `${prop.street} - ${prop.house}`,
                        street: prop.street, houseNumber: getHouseNumber(prop.house),
                        debtId: campaignRef.id, description: formData.description.toUpperCase(),
                        amountUSD, status: 'pending', createdAt: serverTimestamp()
                    });
                }
            }
            await batch.commit();
    ...
    const handleCloseCampaign = async (campaignId: string) => {
        setLoading(true);
        try { await updateDoc(doc(db, 'condominios', condoId, 'extraordinary_campaigns', campaignId), { status: 'closed' }); toast({ title: 'Campaña cerrada' }); }
        catch (error) { toast({ variant: 'destructive', title: 'Error' }); }
        finally { setLoading(false); }
    };
    ...
    const generateCampaignReport = async (campaign: ExtraordinaryCampaign, type: 'pending' | 'paid' | 'all') => {
        ...
        for (const owner of sortedOwners) {
            let debt = null;
            if (type === 'pending') debt = campaignDebts.find(d => d.ownerId === owner.id && (d.status === 'pending' || d.status === 'partial'));
            else if (type === 'paid') debt = campaignDebts.find(d => d.ownerId === owner.id && (d.status === 'paid' || d.status === 'partial'));
            else debt = campaignDebts.find(d => d.ownerId === owner.id);
            if (debt) { filteredDebts.push({ owner, debt, index: counter }); counter++; }
        }
        const stats = getCampaignStats(campaign.id);
        const html = generateReportHTML(filteredDebts, type === 'pending' ? 'POR PAGAR' : type === 'paid' ? 'PAGADOS' : 'LISTA COMPLETA', campaign, stats, type === 'all');
        const fileName = `Cuota_Extraordinaria_${campaign.description.replace(/ /g, '_')}_${type === 'pending' ? 'Por_Pagar' : type === 'paid' ? 'Pagados' : 'Lista_Completa'}_${format(new Date(), 'yyyy_MM_dd')}.pdf`;
        downloadPDF(html, fileName);
    };

    const generateReportHTML = (items: { owner: Owner, debt: OwnerExtraordinaryDebt | null, index: number }[], type: string, campaign: ExtraordinaryCampaign, stats: any, showAllStatus: boolean = false) => {
        const logo = condominioData?.logo || "/logo-condominio-el-valle.png";
        ...
        return `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${campaign.description} - ${type}</title>
        <style>
            *{margin:0;padding:0;box-sizing:border-box}body{font-family:'Helvetica',Arial,sans-serif;margin:20px;padding:20px;background:white}
            ...
            .logo{width:60px;height:60px;border-radius:50%;overflow:hidden;border:2px solid #F5A623}.logo img{width:100%;height:100%;object-fit:cover}
            .company-info{text-align:right}.company-info h2{font-size:16px;font-weight:900;color:#1A1D23;text-transform:uppercase}.company-info p{font-size:10px;color:#64748b}
            h1{text-align:center;font-size:20px;font-weight:900;margin:20px 0;color:#1e293b}
            .campaign-info{text-align:center;margin-bottom:20px}.campaign-info h3{font-size:16px;color:#F28705}
            .summary{display:flex;justify-content:space-between;gap:20px;margin-bottom:20px}
            .summary-card{flex:1;background:#f8fafc;padding:12px;border-radius:8px;text-align:center;border-left:4px solid #F28705}
            .summary-card label{font-size:10px;font-weight:700;text-transform:uppercase;color:#64748b}.summary-card value{font-size:18px;font-weight:900;color:#1e293b}
            table{width:100%;border-collapse:collapse;font-size:10px}th{background:#1A1D23;color:white;padding:10px;text-transform:uppercase;font-size:9px}td{padding:8px;border-bottom:1px solid #e2e8f0;text-align:center}
            .text-left{text-align:left}.text-right{text-align:right}
            .status-pagado{color:#10b981;font-weight:700}.status-parcial{color:#3b82f6;font-weight:700}.status-pendiente{color:#eab308;font-weight:700}
            .footer{margin-top:30px;text-align:center;font-size:8px;color:#94a3b8;border-top:1px solid #e2e8f0;padding-top:10px}
        </style></head><body><div class="container">
        <div class="header"><div class="logo"><img src="${logo}" alt="${nombre}" onerror="this.style.display='none'"></div><div class="company-info"><h2>${nombre}</h2><p>RIF: ${rif}</p></div></div>
        <h1>CUOTA EXTRAORDINARIA</h1><div class="campaign-info"><h3>${campaign.description}</h3><p>Reporte: ${type}</p><p>Generado: ${new Date().toLocaleString('es-VE')}</p></div>
        <div class="summary">
            <div class="summary-card"><label>Monto por Propietario</label><value>$${formatUSD(campaign.amountUSD)}</value></div>
            <div class="summary-card"><label>Total Recaudado</label><value>$${formatUSD(stats.collected)}</value></div>
        ...
        <div className="space-y-10 animate-in fade-in duration-700 font-montserrat italic bg-[#1A1D23] min-h-screen p-4 md:p-8 text-white">
            <div className="mb-6">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-6">
                    <div>
                        <h2 className="text-4xl font-black text-white uppercase tracking-tighter italic drop-shadow-sm">Gestión de <span className="text-primary">Cuotas Extraordinarias</span></h2>
                        <div className="h-1.5 w-20 bg-primary mt-2 rounded-full"></div>
                        <p className="text-white/40 font-bold mt-3 text-sm uppercase tracking-wide">Crear, asignar y monitorear cuotas extraordinarias</p>
                    </div>
                    <Button onClick={() => setIsCreateDialogOpen(true)} className="rounded-xl bg-primary hover:bg-primary/90 text-slate-900 font-black uppercase text-[10px] h-12 px-6 italic">
                                ...
                                <Card key={campaign.id} className="rounded-[2rem] border-none shadow-2xl bg-slate-900 overflow-hidden border border-white/5">
                                    <CardContent className="p-6">
                                        <div className="flex justify-between items-start mb-4">
                                            <div><p className="text-[10px] font-black uppercase text-primary tracking-widest">Campaña Activa</p><p className="font-black text-white text-lg uppercase">{campaign.description}</p><p className="text-[10px] text-white/60">Monto: ${formatUSD(campaign.amountUSD)} USD</p></div>
                                            <Button onClick={() => handleCloseCampaign(campaign.id)} disabled={loading} variant="outline" size="sm" className="rounded-xl border-red-500/30 text-red-400 font-black uppercase text-[10px] hover:bg-red-500/10"><XCircle className="mr-1 h-3 w-3" /> Cerrar</Button>
                                        </div>
