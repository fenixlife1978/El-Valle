...
export default function ReportViewerPage() {
    ...
    const [reportData, setReportData] = useState<any>(null); // Can be FinancialStatement or Integral Report data
        ...
        const fetchReport = async () => {
            ...
            try {
                ...
                if (isBalance) {
                    ...
                    // For integral reports, we need to fetch all necessary data and build it.
                    ...
                    const [ownersSnapshot, debtsSnapshot, paymentsSnapshot, historicalPaymentsSnapshot] = await Promise.all([
                        ownersQuery, debtsQuery, paymentsQuery, historicalPaymentsQuery
                    ]);
            ...
            <div className="space-y-8">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        {reportDate && <p className="text-muted-foreground">Publicado el {format(reportDate, 'dd MMMM, yyyy', { locale: es })}</p>}
                    </div>
                    <Button onClick={handleGeneratePdf}><Download className="mr-2 h-4 w-4"/>Descargar PDF</Button>
                </div>
                 ...
                 <Card>
                    ...
                    <CardContent className="space-y-4">
                        <div className="p-4 bg-primary/10 rounded-md">
                            ...
                            <p id="saldoNeto" className={`text-2xl font-bold text-center ${saldoNeto >= 0 ? 'text-primary' : 'text-destructive'}`}>{formatToTwoDecimals(saldoNeto)}</p>
                        </div>
                 ...
                 <Card>
                    <CardContent className="p-0">
                         <Table>
                            ...
                            <TableBody>
                                {integralData.map(row => (
                                    <TableRow key={row.ownerId}>
                                        <TableCell className="font-medium">{row.name}</TableCell>
                                        <TableCell>
                                            <span className={`font-semibold ${row.status === 'No Solvente' ? 'text-destructive' : 'text-green-600'}`}>{row.status}</span>
                                        </TableCell>
...
const buildIntegralReportData = (
    ...
    const { format, addMonths, startOfMonth, getYear, getMonth, isBefore, endOfMonth } = require('date-fns');

    const sortedOwners = [...owners]
        .filter(owner => owner.id !== 'valle-admin-main-account' && owner.name && owner.name !== 'Valle Admin')
        .map(owner => {
            const propertiesString = (owner.properties || []).map(p => `${p.street}-${p.house}`).join(', ');
            const streetNum = parseInt(String(owner.properties?.[0]?.street || '').replace('Calle ', '') || '999');
            const houseNum = parseInt(String(owner.properties?.[0]?.house || '').replace('Casa ', '') || '999');
